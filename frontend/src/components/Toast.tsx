...
const Toast: React.FC<ToastProps> = ({ id, type, message, subText, onClose, duration = 5000 }) => {
  useEffect(() => {
  ...
  return (
    <div className={`toast-card toast-${type}`}>
      ...
      <div className="toast-message-text-container">
        ...
        {subText && <p className="toast-sub-text">{subText}</p>}
      </div>
      <X size={18} className="toast-cross-icon" onClick={() => onClose(id)} />
    </div>
  );
};
